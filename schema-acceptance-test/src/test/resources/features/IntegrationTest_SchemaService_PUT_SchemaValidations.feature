Feature: To verify functionality of PUT schema Service
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully even if ID, Title, x-osdu and few other optional attributes are added
    ...
    Given I hit schema service PUT API with <BaseInputPayload>, data-partition-id as <tenant> with increased patch version only
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only patch version is increased and Additional properties attribute has false value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased patch version only
    Then user gets patch version error response as <ReponseStatusCode> and <ResponseMessage>

    Examples: #NegativeScenario
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when minor and patch versions are increased and Additional properties attribute has false value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version and patch version
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only minor version is increased and Additional properties attribute has false value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully when only minor version is increased and Additional properties attribute having true(Earlier AP=NA) value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only minor version is increased and Additional properties attribute having true(Earlier AP=False) value
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only patch version is increased and Additional properties attribute having true(Earlier AP=False) value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased patch version only
    Then user gets patch version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only minor version is increased and Additional properties attribute having false(Earlier AP=NA) value
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only patch version is increased and Additional properties attribute having false(Earlier AP=NA) value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased patch version only
    Then user gets patch version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully when only patch version is increased and Additional properties attribute having true(Earlier AP=NA) value
    When I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased patch version only
    Then user gets response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only minor version is increased and Additional properties attribute is absent(Earlier AP=false)
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only patch version is increased and Additional properties attribute is absent(Earlier AP=false)
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased patch version only
    Then user gets patch version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request for adding/replacing any attribute which is not present in the immediate next schema record
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithAddedChanges> and data-partition-id as <tenant> with increased minor version with 2 count
    Then user gets response as <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithReplacedChanges> and data-partition-id as <tenant> with less minor version by 1 count than earlier
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request for removing/replacing any attribute from previous schema record
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithRemovedChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request for removing/replacing any element of OneOf from previous schema record
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds as bad request when only minor version is increased with jumbled oneOf elements and position change of elements on index basis
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets minor version error response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully when only minor version is increased and added one element in oneOf elements
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets response as <ReponseStatusCode> and <ResponseMessage>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully when only minor version is increased with jumbled ref elements in oneOf
    Given I hit schema service POST API with <BaseInputPayload> and data-partition-id as <tenant> and update versions
    Then service should respond back with <ReponseStatusCode1> and <ResponseMessage1>
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant> with increased minor version
    Then user gets response as <ReponseStatusCode1> and <ResponseMessage1>
  ...
  @SchemaService
  Scenario Outline: Verify that Schema Service's PUT API responds successfully when no version is increased with jumbled oneOf elements
    Given I hit schema service PUT API with <InputPayloadWithChanges>, data-partition-id as <tenant>
    Then user gets response as <ReponseStatusCode> and <ResponseMessage>
